...
def get_reviews(restaurant_id: int, db: Session = Depends(get_db)):
    reviews = db.query(models.Review, models.User.username)\
        .join(models.User, models.Review.user == models.User.uid)\
        .filter(models.Review.restaurant == restaurant_id)\
        .all()
...
def get_rating(restaurant_id: int, db: Session = Depends(get_db)):
    reviews = db.query(models.Review).filter(models.Review.restaurant == restaurant_id).all()
    total_rating = 0
    for review in reviews:
...
def create_review(restaurant_id: int, review: schemas.ReviewCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):   
    ...
    new_review = models.Review(restaurant=restaurant_id, user=current_user.uid, rating=review.rating, text=review.text, time=datetime.now())
    db.add(new_review)
    db.commit()
