  ...
  useEffect(() => {
    const fetchRestaurantData = async () => {
      try {
        ...
        const restaurantResponse = await fetch(`http://54.67.35.1/restaurants/${rid}`);
        const restaurantData = await restaurantResponse.json();
        setRating(restaurantData.rating);
        setRestaurant(restaurantData);

        // Fetch menu
        const menuResponse = await fetch(`http://54.67.35.1/restaurants/${rid}/menu`);
        const menuData = await menuResponse.json();
        console.log('Menu data:', menuData); // Add this line
        setMenu(menuData);

        // Fetch reviews
        const reviewsResponse = await fetch(`http://54.67.35.1/restaurants/${rid}/reviews`);
  ...
  return (
    <DetailContainer>
      <RestaurantHeader>
        ...
        <Rating isGood={rating >= 4}>★ {rating.toFixed(1)}</Rating>
      </RestaurantHeader>

      <InfoSection>
        <p>{restaurant.description}</p>
        <p>Address: {restaurant.address}</p>
        <p>Zipcode: {restaurant.zip}</p>
        <p>Hours: {restaurant.opentime} - {restaurant.closetime}</p>
        <p>Price Range: {getPriceRange(restaurant.menu)}</p>
        <ContactInfo>
        ...
        {reviews.length > 0 ? (
            reviews.map((review) => (
                <ReviewCard key={review.rvid}>
                    ...
                    <ReviewDate>{new Date(review.time).toLocaleDateString()}</ReviewDate>
                    </ReviewHeader>
...
const ReviewForm = ({ rid }) => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const response = await fetch(`http://54.67.35.1/restaurants/${rid}/create_review`, {
          method: 'POST',
          headers: {
